...
class Book {
public:
  Book(const Config* config, Logger* logger) {
    file = 0;
    char buf[1024];
  ...
  int find(uint64_t key, char* move) {
    ...
    offset = find_key(file,key,&entry);
    if(entry.key != key) {
      return 1;
    }
    entries[0] = entry;
    count = 1;
    fseek(file,16*(offset+1),SEEK_SET);
    while (1) {
    ...
    for(i = 0; i<count; i++) {
      bestScore +=  entries[i].weight;
      if(int(rand() % bestScore) < entries[i].weight) {
        moveToString(move,entries[i].move);
      }
    }
    return 0;
  }
  ...
  uint64_t hash(char *fen) {
    ...
    p = 0;
    while(1) {
      if((unsigned)p>= strlen(board_s)) break;
      c = board_s[p++];
      if(c == '/') {
        r--;
        f = 0;
    ...
    p = 0;
    while(1) {
      if((unsigned)p>= strlen(castle_flags_s)) break;
      c = castle_flags_s[p++];
      switch(c) {
      case '-' :
        break;
