...
__forceinline int movePiece(const Move move) {
  return (move >> 26) & 15;
}
__forceinline void moveSetPiece(Move& move, const int piece) {
  move |= (piece << 26);
}
__forceinline int moveCaptured(const Move move) {
  return (move >> 22) & 15;
}
__forceinline void moveSetCaptured(Move& move, const int piece) {
  move |= (piece << 22);
}
__forceinline int movePromoted(const Move move) {
  return (move >> 18) & 15;
}
__forceinline void moveSetPromoted(Move& move, const int piece) {
  move |= (piece << 18);
}
__forceinline int moveType(const Move move) {
  return (move >> 12) & 63;
}
__forceinline void moveSetType(Move& move, const int type) {
  move |= (type << 12);
}
__forceinline uint32_t moveFrom(const Move move) {
  return (move >> 6) & 63;
}
__forceinline void moveSetFrom(Move& move, const int sq) {
  move |= (sq << 6);
}
__forceinline uint32_t moveTo(const Move move) {
  return move & 63;
}
__forceinline void moveSetTo(Move& move, const int sq) {
  move |= sq;
}
__forceinline int movePieceType(const Move move) {
  return (move >> 26) & 7;
}
